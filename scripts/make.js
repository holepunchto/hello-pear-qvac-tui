...
const bin = isWindows ? 'hello-pear-qvac.exe' : 'hello-pear-qvac'
const cleanup = []
...
async function make() {
  ...
  if (process.env.KEYCHAIN_PROFILE) {
    const temp = await fs.mkdtemp(path.join(os.tmpdir(), 'hello-pear-qvac-notarize-'))
    cleanup.push(temp)
    const zip = path.join(temp, 'hello-pear-qvac.zip')
